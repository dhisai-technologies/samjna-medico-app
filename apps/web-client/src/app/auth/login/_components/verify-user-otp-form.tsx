...
export function VerifyUserOtpForm({
  email,
  setEmail,
}: { email: string; setEmail: React.Dispatch<React.SetStateAction<string | undefined>> }) {
  ...
  const [state, formAction] = useFormState(verifyUserOtp, null);
  ...
  function handleResendOtp(email: string) {
    ...
    setIsLoading(true);
    toast.promise(resendUserOtp(email), {
